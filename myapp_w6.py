...
@app.route('/records/<bandname>/<albumname>/', methods=['GET'])
def get_band_by_record(bandname, albumname):
	response={bandname:'Not Found!'}
	for item in all_records:
		if item["name"]==bandname:
			for item in all_records:
				if item["albums"]==albumname:
					response = [x["description"] for x in item["albums"]]
					break
	return jsonify(response)
